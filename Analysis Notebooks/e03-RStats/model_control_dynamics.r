...
sink('PushPull_ControllerDynamics_Analysis.txt')

print('Reading Population PushPull')
dat <- read.csv('/Users/akhambhati/Remotes/hoth_research/PeriphData/ds-VCR_PushPull/e03-VirtualResection/Population_ControlDynamics_CType.csv')
dat <- within(dat, {
...
model1 <- lmer('CType_Desync_Transit ~ 1 + (1 | Subject_ID)', data=dat_sel, REML=FALSE)
model2 <- lmer('CType_Desync_Transit ~ 1 + Seizure_ID + (1 | Subject_ID)', data=dat_sel, REML=FALSE)
...
print('CType: Sync')
model1 <- lmer('CType_Sync_Transit ~ 1 + (1 | Subject_ID)', data=dat_sel, REML=FALSE)
model2 <- lmer('CType_Sync_Transit ~ 1 + Seizure_ID + (1 | Subject_ID)', data=dat_sel, REML=FALSE)
print(anova(model1, model2))
print(model2)
print(r.squaredGLMM(model1))
print(r.squaredGLMM(model2))
